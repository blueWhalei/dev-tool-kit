...
| 工具 | 说明 |
|------|------|
| 端口管理 | 本地端口扫描、常用端口、进程终止（Windows） |
| 环境变量 | Windows 用户/系统变量、PATH、备份与导入导出 |
...
| 工具 | 说明 |
|------|------|
| 编码与格式转换 | Base64、URL、JSON、YAML、TOML、时间戳、进制、命名、HTML（统一入口，Tab 记忆） |
| 文本对比 | 逐行 Diff，忽略空白/大小写，统一/并排视图 |
...
| Mock 数据 | 预设模板、按字段生成 JSON、导出 JSON/CSV |
...
|------|------|
| 密码生成 | 自定义规则随机密码 |
| JWT 工具 | Secret 生成、Token 解码、exp/iat 校验、HMAC 验签 |
| Hash 生成 | MD5、SHA-1、SHA-256、SHA-512 |

### 计算器

| 工具 | 说明 |
|------|------|
| Cron 解析 | Cron 表达式解析、接下来 5 次执行时间与相对倒计时 |
| 子网计算 | CIDR 输入，输出网络/广播/掩码/主机范围 |
...
|------|---------|-------|-------|------|
| 端口扫描 | 完整支持 | 完整支持 | 完整支持 | — |
| 终止占用进程 | 完整支持 | 不支持 | 不支持 | 非 Windows 请手动 `kill` |
| 环境变量管理 | 完整支持 | 不支持 | 不支持 | 当前仅 Windows 注册表 |
